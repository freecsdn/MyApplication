...
public class ActionSheet extends Fragment implements OnClickListener {
	...
	private static final String ARG_BUTTON_TITLE = "title_button";
	...
	private void createItems() {
		...
		if(!TextUtils.isEmpty(myTitle)){
			...
			textView.setId(CANCEL_BUTTON_ID + 1000);
			textView.setBackgroundDrawable(mAttrs.otherButtonTopBackground);
			textView.setText(myTitle);
			textView.setClickable(false);
			textView.setTextColor(mAttrs.titilButtonTextColor);
		...
		if (titles != null) {
			for (int i = 0; i < titles.length; i++) {
				...
				bt.setBackgroundDrawable(getOtherButtonBg(titles, i,isTitle));
				bt.setText(titles[i]);
	...
	private Drawable getOtherButtonBg(String[] titles, int i,boolean isTitle) {
		if (titles.length == 1) {
			if(isTitle){
	...
	private Attributes readAttribute() {
		...
		attrs.titilButtonTextColor = a.getColor(R.styleable.ActionSheet_titleButtonTextColor, attrs.titilButtonTextColor);
	...
	private String getMyTitle(){
		return getArguments().getString(ARG_BUTTON_TITLE);
	}
	...
	private static class Attributes {
		...
		public Attributes(Context context) {
			...
			this.titilButtonTextColor = Color.GRAY;
	...
	public static class Builder {
		...
		public Bundle prepareArguments() {
			...
			bundle.putString(ARG_BUTTON_TITLE, myTitle);
			return bundle;
		}
